...
public class DynamicCamera : MonoBehaviour
{
    ...
    {
        // lerp towards player
        gameObject.transform.position = Vector3.Lerp(gameObject.transform.position, new Vector3(player.transform.position.x + cameraOffsetX, player.transform.position.y + cameraOffsetY, -10), Time.deltaTime * lerpSpeed);
    ...
    void CalculateDistanceBetweenPlayers()
    {
        ...
        distanceBetweenPlayers = Mathf.Sqrt(Mathf.Pow(displacement.x, 2) + Mathf.Pow(displacement.y, 2)); //square root of (x^2 + y^2)
    ...
    void CameraFollow2Players()
    {
        ...
        if (distanceBetweenPlayers > 0)   //positive = player1 on the right
            gameObject.transform.position = Vector3.Lerp(gameObject.transform.position, new Vector3(player.transform.position.x - distanceBetweenPlayersV3.x / 2, player.transform.position.y - distanceBetweenPlayersV3.y / 2, -10), Time.deltaTime * lerpSpeed);
        else                                                    //negative = player2 on the right
            gameObject.transform.position = Vector3.Lerp(gameObject.transform.position, new Vector3(focusObj.transform.position.x - distanceBetweenPlayersV3.x / 2, focusObj.transform.position.y - distanceBetweenPlayersV3.y / 2, -10), Time.deltaTime * lerpSpeed);
